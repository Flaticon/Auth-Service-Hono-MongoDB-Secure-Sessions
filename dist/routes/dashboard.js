...
dashboard.use('*', auth_1.authenticateSession);
/**
 * GET /dashboard
...
dashboard.get('/', async (c) => {
    try {
        ...
        const sessionStats = await Session_1.Session.getStats(user._id);
        return c.json({
            success: true,
            data: {
                user: {
...
dashboard.get('/activity', async (c) => {
    try {
        ...
        const sessions = await Session_1.Session.findActiveByUser(user._id);
 ...
 */
dashboard.get('/admin/users', auth_1.requireAdmin, async (c) => {
    try {
        ...
        const result = await User_1.User.findMany(filter, page, limit);
        return c.json({
            success: true,
            data: result
        });
 ...
 */
dashboard.get('/admin/stats', auth_1.requireAdmin, async (c) => {
    try {
        const [userCount, sessionStats] = await Promise.all([
            User_1.User.count(),
            Session_1.Session.getStats()
        ]);
        return c.json({
            success: true,
            data: {
 ...
 */
dashboard.put('/admin/users/:userId/role', auth_1.requireAdmin, async (c) => {
    try {
        ...
        const updated = await User_1.User.updateProfile(userId, { role });
        if (!updated) {
            return c.json({
                success: false,
 ...
 */
dashboard.put('/admin/users/:userId/status', auth_1.requireAdmin, async (c) => {
    try {
        const userId = c.req.param('userId');
        ...
        const updated = isActive
            ? await User_1.User.reactivate(userId)
            : await User_1.User.deactivate(userId);
        if (!updated) {
            return c.json({
                success: false,
                error: 'User not found or failed to update'
 ...
 */
dashboard.get('/admin/sessions', auth_1.requireAdmin, async (c) => {
    try {
        const page = parseInt(c.req.query('page') || '1');
        const limit = parseInt(c.req.query('limit') || '50');
        // Esta función no existe en el modelo, la implementaremos simple
        const sessions = await Session_1.Session.getStats();
        return c.json({
            success: true,
            data: {
